...
5. 编辑人工复核项，补录确认事实。
6. 审核案例库熔炼结果；目录、措辞和写作规律必须可修正、可停用。
7. 根据项目资料、招标要求和已启用案例生成建议任务书。章节数不得写死；8章只是公司常用起点。
8. 允许人工增删、改名和排序章节，并修改每章输入、组件和完成标准。
9. 确认最终《施组编制任务书》；确认版本是下游正文生产的唯一目录。
10. 严格按顺序逐章生成；每章必须人工接受后才能进入下一章。
11. 全部章节确认后装配 DOCX，并用 Word COM 优先导出 PDF。
12. 运行要求覆盖、数字依据、图纸来源、章节一致性和视觉检查。
13. 输出 DOCX、PDF、六份控制报告和 assets。
